...
class VideoStatusLogService:
    ...
    def log_status_change(
        ...
        try:
            ...
            # Get the latest log using a proper query
            latest_log = (
                self.db_session.query(self.status_log_repo.model)
                .filter(self.status_log_repo.model.video_id == video_id)
                .order_by(desc(self.status_log_repo.model.created_at))
                .first()
            )
