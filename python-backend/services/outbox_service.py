...
class OutboxService:
    ...
    def add_to_outbox(
        ...
        repo = GenericRepository(OutboxEvent, session) if db_session else self.outbox_repo
    ...
    def mark_as_published(
        ...
        repo = GenericRepository(OutboxEvent, db_session) if db_session else self.outbox_repo
    ...
    def increment_attempts(self, event_id: UUID) -> None:
        ...
        if event:
            ...
            logger.debug(
                f"📊 Incremented attempts for outbox event: id={event_id}, attempts={event.attempts + 1}"
            )
