...
class VideoTranscriptionService:
    ...
    def transcribe_video(self, video_id: int, video_s3_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 Checking video status for video {video_id}...")
            ...
            logger.info(f"📊 Video {video_id} current status: {video.status}")
            ...
            # 7. Update video status to SUMMARIZING (next stage) - transcription done
            videos_repo = GenericRepository(Videos, self.db_session)
            video = videos_repo.find_one({"id": video_id})
            if video:
                videos_repo.update(
            ...
            try:
                ...
                if video:
                    ...
                    logger.warn(f"⚠️ Video {video_id} not found, cannot update status")
        ...
        for file_path in file_paths:
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    logger.warn(f"Failed to cleanup temp file {file_path}: {str(e)}")
