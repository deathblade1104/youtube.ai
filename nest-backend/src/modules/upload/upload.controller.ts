...
export class UploadController {
  ...
  async initializeUpload(@Body() dto: InitializeUploadDto) {
    const data = await this.uploadService.initializeMultipartUpload(dto);
    return {
      messaage: 'Multipart video upload initialized',
      data,
    };
  }
  ...
  async generatePresignedUrl(@Body() dto: GeneratePresignedUrlDto) {
    return await this.uploadService.generatePresignedUrlForPart(dto);
  }
  ...
  async abortUpload(@Body() dto: AbortUploadDto) {
    return await this.uploadService.abortMultipartUpload(dto);
  }
}
